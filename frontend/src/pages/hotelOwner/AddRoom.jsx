    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key)=>(
          <label htmlFor={`roomImage${key}`}key={key}>
            ...
            <input type="file" accept='images/*' id={`roomImage${key}`} hidden onChange={e=>setimages({...images , [key] : e.target.files[0]})}/>
          </label>
        ))}
      </div>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        ...
        <div>
          ...
          <input type='number' placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24' value={input.pricePerNight} onChange={e=>setInputs({...input,pricePerNight:e.target.value})}/>
          </div>
        </div>
