...
const ListRoom = () => {
  ...
  const fetchRooms = async () => {
    try {
      const { data } = await axios.get("/api/owner/my-rooms", {
        headers: {
      ...
      <div className="w-full max-w-3xl text-left border border-gray-300 rounded-lg max-h-80 overflow-y-scroll mt-3">
        <table className="w-full">
          ...
          <tbody className="text-sm">
            {rooms.map((item) => (
              <tr key={item._id}>
                <td className="py-3 px-4 text-gray-700 border-t border-gray-300 text-left">
                  {item.roomType}
                </td>

                <td className="py-3 px-4 text-gray-700 border-t border-gray-300 max-sm:hidden text-left">
                  {item.amenities?.join(", ")}
                </td>
