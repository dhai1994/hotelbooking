...
return room.pricePerNight >= min && room.pricePerNight <= max;
  ...
  //Filter destinations
  const filterDestination =(room) => {searchParams.get('destination')
  if(!destination) return true;
  return room.hotel.city.toLowerCase().includes(destination.toLowerCase())
  }

  //filter and ort rooms based on selected filter and sort option
    ...
    <div className='pt-28 md:pt-30 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div className='lg:hidden bg-white border border-gray-300 text-gray-600 mb-8'>
        <div className='flex items-center justify-between px-5 py-2.5 border-b border-gray-300'>
          ...
          <span onClick={() => setOpenFilters(!openFilters)} className='text-xs cursor-pointer'>
          ...
          <div>
            ...
            <div className='px-5 pt-5'>
              ...
              {priceRanges.map((range, index) => (
                ...
                selected={selectedFilters.priceRange.includes(range)} onChange={(Checked) => handleFilterChange(Checked, range, 'roomType')} />
              ))}
            </div>
            <div className='px-5 pt-5 pb-5'>
              <p className='font-medium text-gray-800 pb-2'>Sort By</p>
